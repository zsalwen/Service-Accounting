...
if ($_GET[attid]){
	if($_GET[attid] == 'others'){
		$q="select packet_id, date_received, service_status, filing_status, ps_pay.bill410, ps_pay.bill420, ps_pay.bill430, ps_pay.bill440, ps_pay.code410, ps_pay.code420, ps_pay.code430, ps_pay.code440, ps_pay.code410a, ps_pay.code420a, ps_pay.code430a, ps_pay.code440a, ps_pay.code410b, ps_pay.code420b, ps_pay.code430b, ps_pay.code440b, attorneys_id, DATE_FORMAT(date_received,'%M %D, %Y at %l:%i%p') as date_received_f from ps_packets, ps_pay where ps_pay.bill410 <> '' AND date_received < '$pastDue' AND attorneys_id <> '1' AND attorneys_id <> '3' AND ps_packets.packet_id=ps_pay.packetID AND ps_pay.product='OTD' order by packet_id";
	}else{
